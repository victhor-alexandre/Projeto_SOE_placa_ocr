...
cv::Mat processarImagem(const cv::Mat& imagemPlaca) {
    ...
    cv::Canny(imagemFiltrada, imagemLinearizada, 30, 200);
    //std::cout << "Canny aplicado. Tamanho: " << imagemLinearizada.size() << std::endl;

    cv::Mat imagemLimiarizada;
    cv::adaptiveThreshold(imagemLinearizada, imagemLimiarizada, 255, cv::ADAPTIVE_THRESH_GAUSSIAN_C, cv::THRESH_BINARY, 11, 2);
